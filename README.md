...
| 4 | `git clone` this repo, then `npm test` | 1,165 tests, `node:test`, zero dependencies, offline |
...
|---|---|---|---|---|
| **ATLAS** | books.toscrape.com | `c_mt2fnqqngikv29od5` | title, price, rating, image_url, availability | A sandbox built for scraping; no robots.txt, server-rendered. Rating lives in a **CSS class**, not in text — the natural infected-field candidate |
...
npm test                                  # 1,165 tests, zero dependencies, offline
...
*`?mock=1` — the CHAOS LAB. Press BREAK BODEGA and the substance climbs; RE-WEAVE prints
the receipt below.*
...
test/          1,165 tests in {pipeline,web,mcp,tools}/, mirroring the source tree
