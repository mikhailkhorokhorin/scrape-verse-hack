...
Every claim below is checkable from this repository alone. The scan history and the
incident records are committed under [`data/`](data/), the raw Bright Data payloads under
[`evidence/`](evidence/), and `node tools/evidence-report.js` recomputes their SHA-256
digests at call time rather than quoting stored ones.
...
[`create-kestrel.json`](evidence/create-kestrel.json)). Every ID and its per-field
contract is pinned in [`collectors.json`](collectors.json); every heal against those IDs
is in [`data/incidents.json`](data/incidents.json), printable in full with
...
`05:13:45Z` broken, `05:40:09Z` healed), and the heal itself is `inc_001` in
[`data/incidents.json`](data/incidents.json), with four stage timestamps and a per-field
verification block.
...
| | |
|---|---|
| Tests | **1,165**, across 62 files in `test/{pipeline,web,mcp,tools}/` |
...
test/                       1,165 tests, node:test, no dependencies
...
Two ideas from that pass are not in the build, and are named here so they stop being
...
tagline block already carries the product thesis.
...
1,165 tests, `node:test`, no dependencies and no test framework to install:
...
worked example — same `c_mt2fnt3p2k4n644701` before and after. Every heal carries the ID
it ran against in [`data/incidents.json`](data/incidents.json), and
`node tools/evidence-report.js` asserts the two sides are identical before it prints.
...
- **The automated repair path is now proven end to end — it was not, until 22 Aug.** Two
  ...
  so
- **A wrong diagnosis is in the log on purpose.** See `inc_003` above: the diagnosis the
  system believed at the time is still in [`data/incidents.json`](data/incidents.json),
  unedited. Manufacturing cleaner evidence is precisely the failure this project exists to
  expose
...
field-by-field contract is declared per collector in [`collectors.json`](collectors.json).
...
been `null`, on the same Collector ID. The whole trail prints with
`node tools/evidence-report.js inc_001`.
...
- `create` takes 5-25 minutes and `heal` up to 15, and both cost credit. **Never recreate a
  collector that already has an ID** in [`collectors.json`](collectors.json) — if
  `run` fails, `heal` it
