# THWIP â€” a self-healing scraper watch

Scrapers do not crash. They **decay** â€” a site changes a class name, extraction starts
...
WeMakeDevs Ã— Bright Data, 17â€“23 Aug 2026.

**Live console â†’ <https://mikhailkhorokhorin.github.io/scrape-verse-hack/>** Â·
**The manual â†’ [`/manual.html`](https://mikhailkhorokhorin.github.io/scrape-verse-hack/manual.html)** Â·
**Demo video â†’ `<VIDEO>`**

Every scraper is a Spider with an **Integrity** score, 0â€“100: how much of what it
promised came back real. When Integrity drops, a black substance climbs its panel
covering exactly the share that was lost â€” and you can drag it off to read the values
...
```json
{
  ...
  "price": { "value": 51.33, "currency": "GBP", "symbol": "Â£" },
  "rating": "Two",
...
absolute URL, a `rating` must fall in range â€” declared per field in `collectors.json` and
...
It helped me most with **ideas and with parts of the code** â€” sketching approaches,
...
The **console front-end I built by hand**: the comic layout, the symbiote metaphor, the
scratch reveal, the character rig, the diptych and the whole visual system came from me
rather than from a prompt. So did the decisions that make the rest of it worth anything â€”
...
## Self-healing â€” four real breaks, one with nobody watching
...
|---|---|---|---|---|---|
| `inc_001` | KESTREL | `RENAMED` | 0 â†’ 100 | **no** â€” news.ycombinator.com | hand |
| `inc_002` | ATLAS | `DRIFTED` | 90 â†’ 100 | **no** â€” books.toscrape.com | hand |
| `inc_003` | BODEGA | `THROTTLED` | 0 â†’ 100 | yes | the cron, unattended |
| `inc_004` | BODEGA | `RENAMED` | 50 â†’ 100 | yes | **the cron, and no phase was human** |
...
â€” the class names moved, the way a real redesign moves them â€” and then touched nothing.
...
`price: null â†’ Â£18.00` and `rating: null â†’ 4.4`, on an unchanged Collector ID. Nobody ran
...
verdict, and SHA-256 digests **recomputed from the committed files at call time** â€” a
check, not a claim.
...
- verification is computed from a **fresh scrape run after the heal**, never from the
  heal's own report â€” `scripts/verify.js`;
...
`node --test test/pipeline/heal-that-lies.test.js` â€” all nulls, populated garbage,
...
nothing** â€” because nothing on the target had broken. The scraper was returning one
...
stopped â€” which is the point of quoting the commands rather than the totals.
...
| Free â€” reads the committed record | Spends Bright Data credit |
|---|---|
| `fleet_status` Â· `spider_history` Â· `incident_log` | `scan_fleet` â€” scrapes the fleet for real |
| `heal_receipt` Â· `evidence_report` Â· `numbers_audit` | `heal_spider` â€” diagnoses, re-weaves, verifies |

**â†’ [`mcp/README.md`](mcp/README.md)** has the protocol notes, every tool schema, and a
...
![The CHAOS LAB: BREAK BODEGA, RE-WEAVE, TOGGLE UNWATCHED and RESET, above a synthetic fleet of three Spiders](assets/chaos-lab-break-1440.png)

*`?mock=1` â€” the CHAOS LAB. Press BREAK BODEGA and the substance climbs; RE-WEAVE prints
the receipt below.*

![The re-weave receipt: two fields re-checked against the run after the heal, each showing the value before and after, on an unchanged collector id](assets/chaos-lab-receipt-1440.png)

Then open <http://localhost:8000/web/> â€” the console reads two committed JSON files and
...
![The manual: the newspaper advertisement with eight tools and a cut-out coupon carrying the three install commands](assets/manual-1440.png)

*[`/manual.html`](https://mikhailkhorokhorin.github.io/scrape-verse-hack/manual.html) â€”
...
scripts/       the product: scan, score, diagnose, heal, verify
  ...
  verify.js         scores a fresh run after the heal â€” never the heal's own report
web/           the console â€” no build step, no framework, no dependencies
...
demo-target/   the shop page BODEGA watches â€” three variants of the same page
```

No dependencies and no devDependencies: `npm install` installs nothing, and CI asserts
that it stays that way. Every JS and CSS file is **â‰¤250 lines**, enforced as an ESLint
...
| `INFECTED` | arrived and is **wrong** â€” out of range, wrong type, a literal `"undefined"` | half |
...
fetches per catalogue page â€” but every contracted field is already on the catalogue page,
...
- **`REWEAVING` is a state the console can render and nothing writes** â€” `repair.js` runs
...
- **Never recreate a collector that has an ID.** If a run fails, `heal` it â€” a needless
