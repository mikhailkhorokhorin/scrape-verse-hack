...
3. **`npm test`** — 821 tests, `node:test`, zero dependencies, offline, spends nothing.
...
| 8 | No secrets in the repo, in CI logs, or in any video frame | **done in repo, unverified in video** | `.gitignore` covers `.env*`, `credentials.json`, `config.json`, `create-*.json`, `*-probe.json`. The only `BRIGHTDATA_API_KEY` in the tree is `${{ secrets.* }}` in the workflow |
...
`scripts/verify.js` computes it, `web/js/receipt.js` renders it as a two-column
...
- **A character per collector, drawn as inline SVG** (`web/js/rig.js`, `rig-parts.js`) —
...
- **The opening sequence replays a real incident** — `web/js/intro-plan.js` pins
...
- **The incident feed reads as comic issues**, each with a permalink
  (`web/js/issue-route.js` routes off `location.hash`) and a print stylesheet
...
| Test suite | **done** | 821 tests, `npm test`, `node:test`, zero dependencies, offline |
...
- **`data/meta.json` reports 594 tests** — it is regenerated by CI on each build and lags
  the working tree, where `npm test` currently reports 821. Run `npm test` for the live
  number rather than reading the committed file
- **`kestrel-probe.json` and `atlas-probe.json` are gitignored** (`*-probe.json`) and are
  not in a clone. The before/after evidence a judge can actually reach is the
  `verification` object on each incident and `kestrel-after.json`, which is tracked. Do not
  point anyone at the probe files
- **The README header screenshot predates THE HAUL and the characters.**
  `assets/the-watch.png` is accurate but not current
